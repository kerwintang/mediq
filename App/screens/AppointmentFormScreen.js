...
const DumbAppointmentFormScreen = (props) => (
	<View style={{
		...
		{props.showProfileForm?
			<ProfileFormScreen onSubmit={props.showResults} selectPatient={props.selectPatient} cancelPatient={props.closeProfileForm} handleFormChange={props.handleFormChange}/>
...
class AppointmentFormScreen extends Component {
	...
	selectPatient(patient){
		...
		{headers:{"x-access-token": this.props.token}}).then((res) => {
			...
			this.props.closeAppointmentForm();
		}).catch((err) => {
			alert("ERROR: "+err);
			this.props.closeAppointmentForm();
		});
	...
	render() {
		return (
			...
			closeAppointment={this.props.closeAppointmentForm}/>:null
		);
    }
}
